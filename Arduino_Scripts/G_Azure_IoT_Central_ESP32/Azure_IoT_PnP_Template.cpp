...
#define TELEMETRY_PROP_NAME_TURBIDITY_VOLTAGE          "turbidity_voltage"
...
int azure_pnp_handle_command_request(azure_iot_t* azure_iot, command_request_t command)
{
  ...
  if (az_span_is_content_equal(command.command_name, COMMAND_NAME_TOGGLE_LED_1))
  {
    ...
    //LED_Control
    digitalWrite(LED_BUILTIN, led1_on);   // turn the LED On and Off
    //
  }
...
static int generate_telemetry_payload(uint8_t* payload_buffer, size_t payload_buffer_size, size_t* payload_buffer_length)
{
  ...
  int turbidity_adcVal;
  double turbidity_voltage;
  ...
  //Turbidity sensor 
  turbidity_adcVal = analogRead(PIN_ANALOG_IN); 
  turbidity_voltage = turbidity_adcVal * (5.0 / 1024.0);
  ...
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_TURBIDITY_VOLTAGE));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding turbdity_voltage property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, turbidity_voltage, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding turbidity property value to telemetry payload. ");
